    ...
    <div id="new" class="tab-pane fade in active show">
        <div class="category-product-index owl-carousel owl-theme owl-loaded owl-drag">
            ...
            <?php    
                ...
                foreach($quick_search_data as $each)
                {
                    if($user_id !== $each['id'])
                    {                       
                        if($user_data['looking_for'] !== $each['looking_for'])
                        { 
                            if($user_data['age_range_from'] == $each['age_range_from'] || $user_data['age_range_to'] == $each['age_range_to'] || $user_data['community'] == $each['community'] || $user_data['marital_status'] == $each['marital_status'] || $user_data['location'] == $each['location'])
                            {       
            ?>
